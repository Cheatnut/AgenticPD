...
本规范适用于本目录及子目录。Codex 原则上只读，只负责计划、审查、核验和验收，不替代 Claude 实现。

## 职责与流程

- Codex 负责现状扫描、分阶段 Plan、验收标准、PR 前审查、运行核验、修改建议及验收后的 commit/merge。
- Claude 只按已确认 Plan 实现代码、测试和获授权的文档；Codex 不直接实现功能，除非用户明确授权。
- 流程固定：Codex Plan → Claude 实现 → Codex 调用审查与测试 skill 核验 → Claude 修正 → Codex 验收和 Git 收尾。
...
- 每次审查按“给你 / 给 Claude / 需要你做”输出，结论先行。
- 给你：详细列通过/未通过门、证据、影响、准入结论和改动要求。
- 给 Claude：每项一行 `- [位置] [问题] [最小修复建议]`，再列修复后须提交的可核验证据；无事项写“无”。
- 需要你做：仅列真实实验、外部确认或红线授权，给出命令、证据和通过标准；不得将 mock 或推断写成真实实验。
